...
function PlanSummary({ settings, locale, copy, onOpenPlan }: { settings: AppSettings; locale: AppLocale; copy: ReturnType<typeof copyFor>; onOpenPlan: () => void }) {
  const today = useMemo(() => new Date(), []);
  const target = settings.planTarget ?? { targetUseDate: settings.endDate ?? "", needFullAmount: true };
  const targetAmount = !target.needFullAmount && Number.isFinite(target.partialNeedEuro) && (target.partialNeedEuro ?? 0) > 0 ? target.partialNeedEuro ?? null : null;
  ...
  const hasGoal = targetAmount !== null && yearsLeft !== null;
  const deRiskYears = 5;
...
export default function SettingsCboWorkspace(props: Props) {
  ...
  return <div className="settings-cbo p40-settings">
    ...
    {props.activeTab === "general" ? <div className="p40-panel" role="tabpanel">
      ...
      <PlanSummary settings={props.settings} locale={props.locale} copy={copy} onOpenPlan={() => setSheet("plan")} />
    ...
    {sheet === "plan" ? <Sheet title={copy.plan} closeLabel={copy.close} onClose={() => setSheet(null)}><div className="p40-sheet-fields"><label><span>{copy.targetDate}</span><input type="date" value={target.targetUseDate} onChange={(event) => editTarget({ targetUseDate: event.target.value })} /></label><label className="p40-toggle-row"><span><strong>{copy.fullAmount}</strong></span><input type="checkbox" checked={target.needFullAmount} onChange={(event) => editTarget({ needFullAmount: event.target.checked, partialNeedEuro: event.target.checked ? undefined : target.partialNeedEuro })} /></label>{!target.needFullAmount ? <label><span>{copy.targetAmount}</span><input inputMode="decimal" type="number" min="0" value={target.partialNeedEuro ?? ""} onChange={(event) => editTarget({ partialNeedEuro: parseNumber(event.target.value) })} /></label> : null}<div><span>{copy.safeWindow}</span><div className="p40-segments">{[3, 5, 7].map((value) => <button key={value} type="button" className={deRiskYears === value ? "selected" : ""} onClick={() => setDeRiskYears(value)}>{value} {props.locale === "de" ? "J." : "năm"}</button>)}</div></div><label><span>{copy.transferRate}</span><select value={transferRate} onChange={(event) => setTransferRate(Number(event.target.value))}><option value={8}>8%</option><option value={12}>12%</option><option value={16}>16%</option></select></label></div><div className="p40-milestones"><div><span>{copy.milestones[0]}</span><strong>{currentYear}</strong></div><div><span>{copy.milestones[1]}</span><strong>{Math.max(currentYear, goalYear - deRiskYears)}</strong></div><div><span>{copy.milestones[2]}</span><strong>{goalYear}</strong></div></div><button type="button" className="p40-advanced-link" onClick={() => setSheet("simulation")}>{copy.advanced}<small>{copy.advancedHelp}</small><IconChevronRight aria-hidden /></button><button type="button" className="p40-sheet-done" onClick={() => setSheet(null)}>{copy.save}</button></Sheet> : null}
