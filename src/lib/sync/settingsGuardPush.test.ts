...
describe("guarded settings push never clobbers the server emergency profile", () => {
  it("turns a stale settings upsert into a conflict and preserves the remote notfallmappe", async () => {
    // Ban CUC BO cu (notfallmappe RONG) dua tren version 5.
    await db.settings.put({ id: "settings", notfallmappe: "", planName: "Local cu", version: 6 } as never);
    // Server da co Ho so khan cap o version moi hon (6).
    setRemote("settings", "settings", { id: "settings", notfallmappe: CANARY, planName: "Server" }, 6);
    await enqueueOutbox("settings", "settings", "upsert", { id: "settings", notfallmappe: "", planName: "Local cu", version: 6 }, 6, { expectedRemoteVersion: 5 });
    ...
    expect((remote("settings", "settings")?.data as { notfallmappe?: string }).notfallmappe).toBe(CANARY);
  ...
  it("applies a matching guarded settings upsert without any unconditional upsert", async () => {
    await db.settings.put({ id: "settings", notfallmappe: CANARY, planName: "Da cap nhat", version: 6 } as never);
    setRemote("settings", "settings", { id: "settings", notfallmappe: CANARY }, 5);
    await enqueueOutbox("settings", "settings", "upsert", { id: "settings", notfallmappe: CANARY, planName: "Da cap nhat", version: 6 }, 6, { expectedRemoteVersion: 5 });
