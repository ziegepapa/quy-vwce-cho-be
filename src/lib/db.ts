...
type SettingsWithVersion = AppSettings & { version: number };

/**
 * Policy A — single entry point for manual quotes:
 * 1. Validate ISIN/price/asOf (price must be > 0).
 * 2. One Dexie transaction returns typed results:
 *    ensure minimal Instrument, put Quote, and for VWCE mirror
 *    latestVwcePrice/latestPriceDate + enqueue settings outbox.
 * 3. instruments/quotes remain local-only (never outbox).
 *
 * Local save and outbox share the same transaction so a failed outbox
 * write rolls back quote/settings (no "saved locally but UI says fail").
 */
export async function saveManualQuoteForIsin(
  ...
  const result = await db.transaction(
    "rw",
    [db.instruments, db.quotes, db.settings, db.outbox],
    async (): Promise<ManualQuoteSaveResult> => {
      ...
      const quote: Quote = {
        ...
        instrumentIsin: isin,
        currency,
        venue: input.venue ?? existingQ?.venue ?? instrument.venue,
        price: input.price,
        asOf,
        source: "manual",
      ...
      if (isin === VWCE_ISIN) {
        ...
        const ver = prevVer + 1;
        const settingsNext: SettingsWithVersion = {
          ...current,
          id: "settings",
        ...
        // Outbox inside the same transaction so failure rolls back local mirror
        if (syncSettings) {
