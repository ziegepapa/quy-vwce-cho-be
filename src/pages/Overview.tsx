...
export default function Overview(_props: { displayName?: string }) {
  ...
  const ratio =
    total > 0 ? Math.min(100, Math.max(0, Math.round((vwceValue / total) * 100))) : 0;
  const pnlPct =
  ...
  return (
    <div className="ov">
        ...
        {mode === "empty" ? (
          ...
          <>
            ...
            <div
              ...
              aria-label={
                cashNegative
                  ? "Tỉ lệ chưa tính được — số dư âm"
                  : `VWCE ${ratio}%, an toàn ${100 - ratio}%`
              }
            >
              {cashNegative ? (
                <div className="alloc-seg-v8 cash" style={{ flex: 1 }} />
              ) : (
                <>
      ...
      {mode !== "empty" && cashNegative && (
        <div className="card" style={{ marginTop: 12 }}>
          <p style={{ margin: 0, fontWeight: 600 }}>Số dư an toàn đang âm</p>
          <p className="muted" style={{ margin: "6px 0 12px", fontSize: 14 }}>
            Nghĩa là có giao dịch mua hoặc chi nhiều hơn số tiền đã nạp. Hãy kiểm tra xem
            có thiếu giao dịch nạp tiền nào không.
          </p>
          <Link to="/transactions" className="secondary" style={{ display: "inline-flex", minHeight: 44, alignItems: "center" }}>
            Xem giao dịch
          </Link>
        </div>
      )}
