...
export default function HouseholdHandoff({ syncStatus, pending }: { syncStatus: SyncStatus; pending: number }) {
  ...
  const printSnapshot = () => {
    ...
    printContinuitySnapshot({
      locale,
      snapshot,
      labels: { title: text.title, generatedAt: text.generatedAt, localOnly: text.localPrint, plan: text.plan, useDate: text.useDate, planStatus: text.plan, yearsLeft: text.yearsLeftLabel, readiness: text.readinessTitle, sync: text.sync, pending: text.pendingLabel, notConfigured: text.notConfigured },
  ...
  return <main className="hh-shell" aria-label={text.title}>
    ...
    <section className="hh-card hh-snapshot"><div className="hh-card-head"><h2>{text.snapshot}</h2><span>{text.localOnly}</span></div><p>{text.snapshotHint}</p><button type="button" className="hh-snapshot-button" onClick={printSnapshot}>{text.snapshot}</button></section>
